...
# 2. Defining a 2-layer Spiking Neural Network in [Norse](https://github.com/norse/norse) (based on PyTorch)
#    from three NIR primitives: **Linear**, **LIF**, and **LI**.
...
HIDDEN = 128        # LIF hidden neurons
TAU_LIF = 10.0      # LIF membrane time constant [timesteps]
TAU_LI = 10.0       # LI readout time constant [timesteps]
...
# %%
lif_params = norse.LIFBoxParameters(
    tau_mem_inv=torch.tensor([1.0 / TAU_LIF]),
    v_th=torch.tensor([V_TH]),
    v_reset=torch.tensor([0.0]),
    v_leak=torch.tensor([0.0]),
)
li_params = norse.LIBoxParameters(
    tau_mem_inv=torch.tensor([1.0 / TAU_LI]),
    v_leak=torch.tensor([0.0]),
)
...
core = norse.SequentialState(
    ...
    norse.LIFBoxCell(lif_params, dt=DT),        # node 1 — LIF
    torch.nn.Linear(HIDDEN, 10, bias=False),    # node 2 — Linear
    norse.LIBoxCell(li_params, dt=DT),          # node 3 — LI readout
)
...
# `Linear -> nir.Affine`, `LIFBoxCell -> nir.LIF`, `LIBoxCell -> nir.LI`. We
# export `core` (not `model`) — `Lift` is only a temporal wrapper with no NIR
# representation. Passing `dt = 1.0` makes the exported `tau` come out in
# timestep units (`tau = TAU_*`).
#
...
# factor into the weights of the Linear layer feeding the LI readout
# (`alpha_LI = 1 - dt/TAU_LI`, so the factor is `1/TAU_LI`). The LIF needs no
# such fix — its `norm_input=True` convention already matches Norse.

# %%
sample = rate_encode(test_images[:1], TIMESTEPS)[0]  # (1, 784) — a single timestep
graph = norse.to_nir(core, sample_data=sample, dt=DT)

# Fold the LI norm_input factor into its incoming Linear weights.
li_id = next(nid for nid, node in graph.nodes.items() if isinstance(node, nir.LI))
...
state = None
linear1, lif_out, linear2, li_out = [], [], [], []
lif_v, li_v = [], []
with torch.no_grad():
    for t in range(TIMESTEPS):
        hidden, state = core(rep[t], state)
        linear1.append(hidden[0])
        lif_out.append(hidden[1])
        linear2.append(hidden[2])
        li_out.append(hidden[3])
        lif_v.append(state[1].v)
        li_v.append(state[3].v)
...
np.savez(
    ...
    lif_output=_stack(lif_out),          # (T, 128)
    lif_v_mem=_stack(lif_v),             # (T, 128)
    linear2_output=_stack(linear2),      # (T, 10)
    li_output=_stack(li_out),            # (T, 10)
    li_v_mem=_stack(li_v),               # (T, 10)
