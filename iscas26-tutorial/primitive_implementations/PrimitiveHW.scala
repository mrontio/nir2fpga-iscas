    ...
    params match {
      ...
      case p: LinearParams =>
        val outSize = p.weight.shape.headOption.getOrElse(
          throw new Exception(s"LinearParams for node '$id' has invalid weight shape ${p.weight.shape}")
        )
        val zeroBias = Tensor(Array.fill(outSize)(0.0f), List(outSize))
        AffineHW(id, AffineParams(bias = zeroBias, weight = p.weight), config)
